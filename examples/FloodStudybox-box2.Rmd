...
t6 = 2000000
dt6 <- dts(t6,rundata)
d6r <- dt6[dt5$pp_SelfReplicator,]
head(dttab(dt6,pp=T),n=7)
```

We need to find the origin of the sequence "$=?>^AC". It's not a *product* - its a modification of an input moleulce, so harder to find. First we need to add columns to the data to get these modified sequences: 

```{r}
msg <- require(Rstringmol)
dt6$aout = ""
dt6$pout = ""
for(rr in 1:nrow(dt6)){
  result <- runReactionFP(c(dt6$actseq[rr],dt6$passeq[rr]))
  dt6$aout[rr] = result$mActive[1]
  dt6$pout[rr] = result$mPassive[1]
}
```
