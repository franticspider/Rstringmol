...
We can now load this data and get all the reactions that are classed as parasitic, using this code: 
...
let's get a list of the unique reactions, and plot a histogram of them compared with the parasitic reactions: 
...
sm_lap_plot <- function(data,xlim=range(c(data$actlen,data$paslen))){
 ...
 pcols <- rainbow(n = nrow(data))
  ...
  plot(NA,xlim=xlim,ylim=c(5,25),yaxt="n")
...
OK, there's a few things to note in this plot - I'll try to discuss these in the order of the plots above:

- The active species seem to be in three distinct size groups with a long 'tail' of lengths above 40 opcodes. I think that the rightmost peak and the tail are long replicators, the middle peak is the pre-flood replicator and the leftmost peak represent parasite-parasite binding. 
- You can see that the distribution of all passive species is very similar, if slightly skewed towards shorter species - this is entirely to be expected I guess. Note that where binding is generally non-complementary (so `$=?>$` binds to `$=?>$`) it becomes much harder to distinguish between the altruistic and parasitic strategies, since assignment of control of the program is 50:50 on binding.
...
The plot above shows that even at this point, the majority of parasitic reactions are confined to the remaining short replicators. 
...
OK, let's check the short active molecules first - are they really parasites? 

```{r}
lpur <- pur[pur$actlen>30,]
```

We can write the steps of a raw reaction to a file like this - then step through it manually to get the diagrams that we need: 
