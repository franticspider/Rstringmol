...
Before the flood, the arena looks like \ref{fig:fig1}. - note the preponderance of pale blue in the length plot - these are what well call "short replicators": 



```{r fig1, echo=FALSE, fig.cap="State of the arena before the flood event",out.width=300, fig.align = 'center'}
knitr::include_graphics("/home/sjh/Desktop/paulien/smsp/1705smsp/out3R/rtypes0000075.png")
```
...
draw_reaction <- function(data,acty,pasy,pcols){
  ...
  points(x=ja,y=rep(acty,length(data$actlen)),col=pcols,pch=19,cex=(1+log(data$nobs)))
  points(x=jp,y=rep(pasy,length(data$actlen)),col=pcols,pch=19,cex=(1+log(data$nobs)))
  segments(x0=ja,x1=jp,y0=rep(acty,length(data$actlen)),y1=rep(pasy,length(data$actlen)),col=pcols,lwd = ((1+log(data$nobs))/2))
...
The plot above shows each reaction as a pair of dots joined by a line. Active are on the top row and passive are on the bottom, with a line connecting them. The size of the dots is proportional on a log scale to the number of times the reaction is occurring at this particular timestep.  This shows nicely that the majority of parasitic reactions are confined to the remaining short replicators. 
...
#sink()
lpur <- lpur[order(lpur$nobs,decreasing = T),]
```

Here's a quick analysis of the five parasitic reactions on replicators longer than 30: 

- This one has a parasite that sends execution back to the replicator becuase it is a copy of most of the replictor:
```
A	$=?>$EBUB^A$=?>$AVB$=?>$EBUB^B$=?>$AVBO%}HOB	
P $=?>$EBUB^A$=?>$AVB$=?>$EBUB^B
```

- This one has lost the `^` opcode that is needed to move execution to the passive molecule
```
           |
           v
A	$=?>$EBUBA$=?>$AVB$=?>$EBUB^B$=?>$AVBO%}HOB	
P $=?>$EBUB^A$=?>$AVB$=?>$EBUB^B$=?>$AVBO%}HOB
```

- This one is parasitic because there's a `^B` instead of an `^A` so the execution switch doesn't happen
```
            |
            v
A	$=?>$EBUB^B$=?>$AVBO%}HOB$=?>$EBUB^B$=?>$AVBO%NHOB	
P $=?>$EB
```

- The parasite here is a replicator with a broken copy loop, so it can't replicate but it can parasitise
```
A	$=?>$EBUB^A$=?>$AVB$=?>$EBUB^B$=?>$AVBO%}HOB	
P $=?>$EBUB^A$=?>$AVB$=?>$EBUB^$=?>$AVBO%}HOB
                               ^
                               |
```

- This parasite is longer than the replicator!
```
A	$=?>$EBUB^A$=?>$AVB$=?>$EBUB^B$=?>$AVBO%}HOB
P $=?>$EBUB^A$=?>$AVB$=?>$EBUB^B$=?>$AVB}%}HOB$=?>$EBUB^A$=?>$AVB$=?>$EBUB^B$=?>$AVB
```

In summary then, I think we can say that the parasitism observed during a flood follows one of two strategies: 

- Either parastise the shorter ("non-checking") replicators with a shorter parasite. This is an 'echo' of the earlier dominant ecosystem which never fully goes away
- Fool the long ("checking") replicators into copying you by having extra code to evade the check. 


# Change in length-dependent parasitism

Here we are using length as a proxy for function of the replicator - let's create a big figure that shows how this evolves from t=150,000


```{r}
pdf(file="parsite_evolution.pdf",width = 8, height = 8)
for(tt in seq(from = 20000, to = 2000000, by = 20000)){
  message(sprintf("%d",tt))
  fdata <- rconf_rdata(sprintf("~/Desktop/paulien/smsp/1705smsp/out3/out1_%d.conf",tt))
