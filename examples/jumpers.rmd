...
```{r fig.height=10, fig.width=10}
  cf <- read.table(sprintf("%sout1_%d.conf",froot,740000),as.is = T, fill = T, sep=" ",comment.char="",stringsAsFactors = F)
hits <- get_pos_plot(cf,740000,c(1,10,15,20))
```


R-R reaction takes 171 steps
...
```{r fig.height=10, fig.width=10}
  cf <- read.table(sprintf("%sout1_%d.conf",froot,1600000),as.is = T, fill = T, sep=" ",comment.char="",stringsAsFactors = F)
...
# Flood in run 2 at 0:46 (t~=1,108,434)

Let's load the files and plot them around this


```{r}
froot <- "~/Desktop/paulien/smsp/1705smsp/out2/"
froot <- "D:/sjh/stringmol/smsp/1705smsp/out2/"
for(t in seq(1000000,1200000,20000)){
...
```{r fig.height=10, fig.width=10}
  cf <- read.table(sprintf("%sout1_%d.conf",froot,1120000),as.is = T, fill = T, sep=" ",comment.char="",stringsAsFactors = F)
hits <- get_pos_plot(cf,1120000,c(30,40,80,90))
```

AGENT  $=?>$UO^A$T>D$VOO^BC=?>$BLUO%}=YH 1 Q
GRIDPOS  42 31
...
GRIDPOS  44 33   
 ...
 - Bind p 0.644
 - Self-scan in 98 steps
 - Toggle to passive, step 7 
 - Toggle to active F to `$T`
 - Locked in infinite loop! Reaction never ends!
