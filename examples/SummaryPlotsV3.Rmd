...
APLenPlot <- function(rundata,title,parasites=F){
  ...
  for(rr in 1:length(rundata)){
    pr <- rundata[[rr]]
    if(parasites)
      pr <- pr[pr$np_Parasite,]
    else
      pr <- pr[pr$np_MutualRepl,]
...
plotplots <- function(rd,divd,title){
  ...
  APLenPlot(rundata,title=sprintf("%s, Mutual Replications",title))
  APLenPlot(rundata,title=sprintf("%s, Parasitic",title),parasites = T)
  
  par(mar = c(0,0,4,4)+0.1)
  
  sumvp(rundata,"nos",myylim=c(0,6000))
  title("Numbers of bound molecules",line=-4)
  sumvp(rundata,"beta")
  title("Reaction time (beta)",line=-3)
