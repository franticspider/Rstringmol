...
Theres a function called `splist_stats` that grabs the data from the files - let's check it out: 

```{r}
source("../R/splist_stats.R")
sps <- splist_stats(sprintf("%ssplist100000.dat",froot),tmin=90000)
```
This gives us a data frame holding every reaction seen until this point (assuming no restarts). 
...
while(gotdata){
  ...
  if(file.exists(fn)){
    ...
    sps[[nsps]] <- splist_stats(fn,tmin=(tt-10000))
    nsps <- nsps + 1
  }
  else
    gotdata <- F
...
```
That works as far as it goes, but we aren't seeing the *dominant* reaction that we are looking for, just the "most common reaction that first occured within 20000 time steps". 
...
Let's try some code that looks at these records to try to build a picture of what's going on at any particular time.
First, we'll get each reaction: 
```{r}
require (stringr)
...
```{r}
  ...
  #This introduces NAs by coercion sometimes
  for(rr in 1:nr){
  ...
  #summarize each reaction with the active and passive reactants (note we aren't assuming a different product...)
  ...
  for(rr in 1:nrow(reactions)){
    ...
    #Check for NAs in the species numbers: 
    ...
    #Get all reactions with the same active and passive spp
...
reactions <- rconf_stats(fn = sprintf("%sout1_400000.conf",froot), spfn = sprintf("%ssplist400000.dat",froot))
...
while(gotdata ){#&& tt < 320000){#
  ...
  if(file.exists(fn)){
    message(sprintf("\nProcessing Time %d:",tt))
    #reactions <- rconf_stats(fn = sprintf("%sout1_%d.conf",froot,tt), 
    #                         spfn = sprintf("%ssplist%d.dat",froot,tt),verbose=F)
    reactions <- rconf_stats_nosplist(fn = sprintf("%sout1_%d.conf",froot,tt), 
                                      #spfn = sprintf("%ssplist%d.dat",froot,tt),
                                      verbose=T)
    
    #message("\n\nout\n\n")
    
    if(is.null(dim(reactions))){#nrow(reactions)==0){
    }
    else{
...
bcols <- bcols[sample(1:length(bcols))]
par(mfrow=c(2,1))
barplot(t(rcounts[2:9]),col=bcols,border=bcols,names=rcounts[,1],las=2,main="Reaction Types")
cn <-colnames(rcounts)

#NB! This is not currently automatically ordered correctly..
cnlong <- c("Pathological non-self replicator",
            "Non-self reaction; different product",
            "Parasitic",
            "Pathological replicator",
            "self-self; different product",
            "self-self; exact replicator",
            "No Product",
            "Unclassified")

legend("topleft",legend=sprintf("%s %s",rev(cn[2:9]),cnlong),fill=rev(bcols),border=rev(bcols),cex = 0.5)

barplot(t(rsumcounts[2:9]),col=bcols,border=bcols,names=rcounts[,1],las=2,main="Reaction Counts")
cn <-colnames(rcounts)

legend("topleft",sprintf("%s %s",rev(cn[2:9]),cnlong),fill=rev(bcols),border=rev(bcols),cex = 0.5)
...
#rcd <- rconf_rdata(fn,verbose=T)


reactions <- rconf_stats(fn, spfn, verbose = F)

```

http://127.0.0.1:22803/help/library/stringr/html/str_extract.html
