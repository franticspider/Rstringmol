...
####Function 'arguments'
  ...
  verbose <- T
  spldata <-  splist_stats(sprintf("%ssplist200000.dat",froot))
    ...
    if(reactions$actno[rr] == reactions$pasno[rr]){
      ...
      if(nrow(dd)==1){
        ...
        reactions$type[rr]= "PA"
          ...
          #message(sprintf("none-self reaction with different product (DP)*",rr))
...
reactions <- rconf_stats(fn = sprintf("%sout1_200000.conf",froot), spfn = sprintf("%ssplist200000.dat",froot))
```
...
tt <- 10000
#rcounts <- data.frame(rclass)
rcounts <- data.frame(time=0,"NA"=0,NP=0,ER=500,DP=0,PR=0,PA=0,DN=0,PN=0)
...
while(gotdata){
  ...
  if(file.exists(fn)){
    ...
    for(rr in 1:length(rclass)){
      ...
      dd <- reactions$type[reactions$type == rclass[rr]]
      count[rr] <- length(dd)
    }
